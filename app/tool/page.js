...
export default function ToolPage() {
  ...
  function runPredictions(m, heightIn) {
    const preds = BRAND_LIST.map(b => {
      ...
      return {
        brand: b,
        top: predict(tA, m, false, null, gender),
        bot: predict(bA, m, true, heightIn, gender),
      };
  ...
  function handleLookup() {
    ...
    setTimeout(() => {
      ...
      const preds = BRAND_LIST.map(b => {
        ...
        return { brand: b, top: predict(tA, m, false, null, profile.gender), bot: predict(bA, m, true, hVal, profile.gender) };
      }).filter(p => p.top.s !== "N/A" || p.bot.s !== "N/A");
      preds.sort((a, b) => Math.max(b.top.conf, b.bot.conf) - Math.max(a.top.conf, a.bot.conf));
